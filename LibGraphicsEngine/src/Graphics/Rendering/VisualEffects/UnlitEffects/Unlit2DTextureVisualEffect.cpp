...
Unlit2DTextureVisualEffect::Unlit2DTextureVisualEffect(Texture2D* pTexture2D)
...
Unlit2DTextureVisualEffect::~Unlit2DTextureVisualEffect()
{}

void Unlit2DTextureVisualEffect::InitCustomEffect()
{
	...
	auto* pVertexShader = GE_ALLOC(Shader)(std::string() + GE_ASSET_PATH + "shaders/VisualEffects/UnlitEffects/unlit2DTexture.vert");
	assert(pVertexShader != nullptr);
	auto* pFragmentShader = GE_ALLOC(Shader)(std::string() + GE_ASSET_PATH + "shaders/VisualEffects/UnlitEffects/unlit2DTexture.frag");
	...
	auto it = uniforms.find(GLSLShaderTypes::Constants::UNIFORM_2D_TEXTURE);
	if (it != uniforms.end() && it->second.type == GLSLShaderTypes::Constants::SAMPLER_2D_TYPE)
	{
